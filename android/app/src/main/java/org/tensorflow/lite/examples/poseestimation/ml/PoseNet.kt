...
class PoseNet(private val interpreter: Interpreter, private var gpuDelegate: GpuDelegate?) :
    ...
    private fun postProcessModelOuputs(
        ...
        val shoulder= keypointList.get(6)?.coordinate
        val elblow = keypointList.get(8)?.coordinate
        val hip = keypointList.get(12)?.coordinate
        ...
        if (elblow != null) {
            if (shoulder != null && hip != null) {
                ...
                Log.d(PoseNet.TAG, "shoulder Angle: " + shoulderFlexion)
                Log.d(PoseNet.TAG, "Posenet Actual shoulder Angle: " + shoulderAngle)
            }
        };
        return Person(keypointList.toList(), totalScore / numKeypoints, shoulderAngle = shoulderAngle)
    }
