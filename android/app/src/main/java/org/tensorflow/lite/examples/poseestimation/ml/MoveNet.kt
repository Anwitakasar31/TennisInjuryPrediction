...
class MoveNet(private val interpreter: Interpreter, private var gpuDelegate: GpuDelegate?) :
    ...
    override fun estimateSinglePose(bitmap: Bitmap): Person {
        ...
        //Anwita - Detect shoulder angle
        val shoulder= keyPoints?.get(6)?.coordinate
        val elblow = keyPoints?.get(8)?.coordinate
        val hip = keyPoints?.get(12)?.coordinate
        ...
        if (elblow != null) {
            if (shoulder != null && hip != null) {
                ...
                Log.d(TAG, "Movenet Actual shoulder Angle: " + shoulderAngle)
            }
        };
        return Person(
            keyPoints,
            totalScore / numKeyPoints, shoulderAngle
        )
    }
